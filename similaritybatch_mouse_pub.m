function similaritybatch_mouse_pub(folder,id)
format compact
format short g
startdir = cd;
%initialize
...
for i = 1:length(dfiles)
    if ~isempty(findstr(dfiles(i).name, '.wav')) || ~isempty(findstr(dfiles(i).name, '.WAV')) && dfiles(i).isdir==0
        files = [files i];
    end;
end;
...
for i=1:length(s1_files);
    fn=[folder,'/', s1_files(i).name];
    wv=audioread(fn);
%     movefile([folder,'/',s1_files(i).name,'originals'])
...
for j=1:length(Sound2)
    ...
    for i=1:length(Sound1)
        ...
        [dC_g]=diffCoef(Sound1(i).Dl,Sound2(j).Dl,mindur);
        dC_sim(calcnum,:)=dC_g;
